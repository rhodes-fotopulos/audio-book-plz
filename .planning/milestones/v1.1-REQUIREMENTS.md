# Requirements Archive: v1.1 Pipeline Quality Improvements
...
- [x] **LLM-01**: Attribution uses qwen3:14b Q4_K_M with KV cache quantization, falling back to 8B Q8_0 if 14B exceeds 16GB memory budget
- [x] **LLM-02**: Dialogue detection uses hybrid regex + LLM to tag lines as spoken/thought/shouted/whispered
- [x] **LLM-03**: Ollama model loads once at pipeline start and stays resident across all LLM phases, with explicit unload before TTS

### Emotion System

- [x] **EMO-01**: Character extraction generates a voice_baseline field describing each character's default speaking style
- [x] **EMO-02**: Scene mood analysis produces one mood + intensity per scene via LLM pass
- [x] **EMO-03**: Line-level emotion overrides flag only lines where speaker emotion sharply breaks from scene mood
- [x] **EMO-04**: Emotion data flows to synthesis as post-processing parameters (volume/speed adjustments for speech-act tags), not as TTS instruct prompts

### Production Polish

- [x] **POL-01**: Pause timing uses Gaussian-randomized durations within context-aware ranges (scene breaks, speaker changes, chapter breaks)
- [x] **POL-02**: Post-processing applies pedalboard effects chain incrementally: trim silence, noise gate, compress, high-pass EQ at 80Hz, limit
- [x] **POL-03**: Final export is 44.1kHz 192kbps CBR MP3 (ACX spec)
- [x] **POL-04**: Voice consistency pass compares same-speaker segment embeddings and regenerates outliers (max 3 attempts, cosine threshold starting at 0.60)
- [x] **POL-05**: Segments use 5-10ms fade-in/fade-out crossfades instead of hard silence insertion
...
| LLM-01 | Phase 8, 10 | Complete |
| LLM-02 | Phase 8, 10 | Complete |
| LLM-03 | Phase 8, 10 | Complete |
| EMO-01 | Phase 8, 10 | Complete |
| EMO-02 | Phase 8, 10 | Complete |
| EMO-03 | Phase 8, 10 | Complete |
| EMO-04 | Phase 8, 10 | Complete |
| POL-01 | Phase 9, 10 | Complete |
| POL-02 | Phase 9, 10 | Complete |
| POL-03 | Phase 9, 10 | Complete |
| POL-04 | Phase 9, 10 | Complete |
| POL-05 | Phase 9, 10 | Complete |
